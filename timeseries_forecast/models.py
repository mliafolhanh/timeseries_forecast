...
class TimeseriesForecastingModel(ABC):
    ...
    def measure_error_2(self, test_series, preds, flag = None):
        """
        Get the error information when giving prediction and actual value
        ...
        if flag == 'agg':
            ...
            agg_test_series = pd.DataFrame([test_series[self.target_col[0]].sum()], columns = self.target_col)
            agg_preds = pd.Series(preds.sum(), index = [preds.index[-1]])
            end_time = time.time()
            print('Agg time', end_time - start_time)
            return self.measure_error(agg_test_series, agg_preds)
        else:
            return self.measure_error(test_series, preds)
...
class DriftModel(TimeseriesForecastingModel):
    ...
    def fit(self):
        ...
        self.resid = pd.Series(y-self.model.predict(X).ravel(), self.train_series.index)
